...
/*
 * Linux/RK3588 兼容实现：
 * 当前工程在 Linux 用户态下运行 demo，因此这里不做真实的中断屏蔽和 PendSV 切换。
 * 如果迁移到 MCU/RTOS，这个头通常是最先需要按平台重写的地方之一。
 */
 ...
 * 保留这些宏的主要价值是让上层代码继续保持可移植的“临界区写法”。
 */
...
/*
 * 这两个函数属于“平台相关的任务启动与初始栈构造”。
 * 在 Linux demo 里是兼容桩；在真实嵌入式端口里则会直接决定任务怎么被切入执行。
 */
